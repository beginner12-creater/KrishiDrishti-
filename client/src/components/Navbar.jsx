...
export default function Navbar({ onOpenReportModal, activeVillage, isDarkMode, onToggleTheme, currentHour = 12 }) {
  ...
  const getHourlyTimeBadge = () => {
    if (currentHour >= 5 && currentHour < 8) {
      return { label: '🌅 Sunrise', color: 'text-amber-500 bg-amber-500/10 border-amber-500/30' };
    }
    if (currentHour >= 8 && currentHour < 17) {
      return { label: '☀️ Daytime', color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/30' };
    }
    if (currentHour >= 17 && currentHour < 20) {
      return { label: '🌇 Sunset', color: 'text-orange-500 bg-orange-500/10 border-orange-500/30' };
    }
    return { label: '🌙 Night', color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/30' };
  };
      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-2 w-full min-w-0">
          ...
          <div className="min-w-0 truncate">
            <div className="flex items-center space-x-1.5">
              <h1 className={`text-base sm:text-xl font-black leading-none truncate ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                KrishiDrishti <span className="text-emerald-600 text-sm font-black">AI</span>
              </h1>
              <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-black border hidden sm:inline-block whitespace-nowrap ${timeBadge.color}`}>
                {timeBadge.label}
              </span>
            </div>
            <p className={`text-[11px] font-extrabold truncate ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
              Climate Risk & Agronomic Decision Platform
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 shrink-0">
          ...
          <button
            ...
            className={`min-h-[44px] min-w-[44px] px-3 py-2 rounded-xl border transition-all duration-300 flex items-center justify-center space-x-1.5 cursor-pointer font-black text-xs ${
              isDarkMode
          ...
          {activeVillage && (
            <button
              ...
              className="min-h-[44px] px-4 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-black text-xs flex items-center space-x-1.5 transition-all cursor-pointer shadow-md active:scale-95"
